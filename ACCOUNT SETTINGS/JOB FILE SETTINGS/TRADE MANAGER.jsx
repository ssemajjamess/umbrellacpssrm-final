...
<head>
    ...
    <meta charset="utf-8" />
    <title ng-init='title = "Trade Manager"' ng-bind="title + ' &middot; AccuLynx'">Trade Manager &middot; AccuLynx</title>
...
<header id="vue-header" class="webapp-header" data-bootstrap="{&quot;Tasks&quot;:0,&quot;TasksDueToday&quot;:0,&quot;TasksOverdue&quot;:0,&quot;Appointments&quot;:0,&quot;Reminders&quot;:0,&quot;RemindersToday&quot;:0,&quot;AtMes&quot;:16,&quot;AtMeFiltersAllowed&quot;:true,&quot;CompanyUserID&quot;:&quot;2bde0f37-77ca-4733-9b4e-aaca2078a0fd&quot;}">
    <ax-app>
        ...
        <!--help modals-->
        <ax-modal v-model="showSubmitAnIdeaModal" :component="submitAnIdeaModalView" :data="{'ViewAllIdeasURL': 'http://feedback.acculynx.com/forums/243621-general-feedback/?sso=yKNHDwZQdVXYO2TtEC5e8El1b6Oy1VlDCOhh%2f0Tr7R91OR2QJHXLAsQGiP%2fIUjymVANUIDPLiIQMWKpaZGN%2bAo0YyFRRzxcSJ7kkEtSlX47tpehNqxp22Cn%2b1lis4HuDLI6%2f1crq9vx4Lw%2fYx38waxOTOuZ%2b%2blNIpAzU%2fpD3bvml2KiBRB2XyxL4KMuKG7jZ%2fcSw5rl%2fi4RCHWwfXal1Zg%3d%3d'}"></ax-modal>
...
<div ng-controller="TradeManagerCtrl as ctrl">
        ...
        <div class="flex-grow account-settings__main-content" style="max-width: 720px">
            <section class="padded widget">
                <div class="account-settings__refinements flex flex-between-center">
                    <div class="flex flex-start-center">
                        <div class="branded-search account-settings__refinement-col">
                            ...
                            <input type="search" ng-change="ctrl.UpdateList();" name="search" placeholder="Search" tabindex="3" ng-model="ctrl.filterText">
                        </div>
                ...
                <header class="widget-header">
                    <h2 class="widget-title">
                        <svg class="icon icon-default">
                            <use xlink:href="/Images/icon-sprite.svg#icon-hammer"></use>
                        </svg>
                        Trade Manager
                    </h2>
                </header>
                <div class="widget-body">
                    <p>
                        Define the trades and corresponding colors to be used throughout AccuLynx (e.g. Roofing, Siding, Gutters).
                    </p>
                    ...
                    <ul class="account-settings__tag-listing">
                        <li class="account-settings__tag flex flex-between-center" ng-class="{'account-settings__tag--edit': trade.isEditing}" ng-repeat="trade in ctrl.FilteredList | filter: ctrl.filterText | orderBy: ['Name']">
                            <div class="flex flex-start-center">
                                <div class="input-checkbox" style="margin-right: 12px;">
                                    <input id="checkbox-input-{{$index}}" ng-model="trade.IsActive" ng-change="ctrl.SaveTrade(trade, 'edit', true)" class="input-checkbox__input" type="checkbox" checked="checked" />
                                    <label for="checkbox-input-{{$index}}" class="input-checkbox__label--horizontal"></label>
                                </div>
                                <span ng-show="!trade.isEditing">{{trade.Name}}</span>
                            </div>
                            <input class="account-settings__tag__name" type="text" ng-model="ctrl.CurrentTrade.Name" value="{{trade.Name}}" ng-show="trade.isEditing" />
                            <div class="flex-noshrink" ng-show="!trade.isEditing">
                                <button class="account-settings__tag__action" ng-click="ctrl.addOrEditTrade(trade)">
                                    <svg class="icon icon-pencil">
                                        <use xlink:href="/Images/icon-sprite.svg#icon-pencil"></use>
                                    </svg>
                                </button>
                                <button ng-disabled="trade.IsUsed" class="account-settings__tag__action" ng-click="ctrl.openDeleteModal(trade)" title="{{trade.IsUsed ? 'Cannot Delete. Currently in use.' : 'Delete'}}">
                                    <svg class="icon icon-trash-can">
                                        <use xlink:href="/Images/icon-sprite.svg#icon-trash-can"></use>
                                    </svg>
                                </button>
                                <!--Production calendar picker if the company is elite-->
                                <span class="account-settings__tag__action" ng-show="ctrl.ProductionCalendarColorList[trade.TradeManagerID]">
                                    <input type="text" id="trade-color-{{trade.TradeManagerID}}" ng-model="ctrl.ProductionCalendarColorList[trade.TradeManagerID]" style="display: none;" ng-change="ctrl.setTradeColor(trade.TradeManagerID)" />
                                    <input class="color {hash:true, slider:false, pickerPosition:'top', pickerFace:0, pickerBorder:0, pickerInset:0, valueElement:'trade-color-{{trade.TradeManagerID}}'}" value="{{ctrl.ProductionCalendarColorList[trade.TradeManagerID]}}" />
                                </span>
                            </div>
                            <div class="flex-noshrink" ng-show="trade.isEditing">
                                <button class="account-settings__tag__action button-solid button" ng-click="ctrl.SaveTrade(trade, 'edit')" ng-disabled="ctrl.isSaving">
                                    Save
                                </button>
                                <button class="account-settings__tag__action button-clear button" ng-click="trade.isEditing = false" ng-disabled="ctrl.isSaving">
                                    Cancel
                                </button>
                            </div>
                        </li>
                    </ul>
                    <div style="border-top: 2px #e0e0e0 dotted"></div>
                    <div class="" ng-show="!ctrl.addingTrade">
                        <button class="button-v3--sm button-v3--text-blue f--16 m-t--md" tabindex="4" ng-click="ctrl.addOrEditTrade(null)">
                            <svg width="14" height="14" class="m-r--md ax-svg ax-svg--plus"><use xlink:href="/Images/icon-sprite.svg#icon-plus"></use></svg>
                            Add Another
                        </button>
                    </div>                  
                    <div class="account-settings__refinements flex flex-between-center" ng-show="ctrl.addingTrade">
                        <div>
                            <span>Add</span>
                            <input id="js-adding-item-name" class="account-settings__tag__name" type="text" ng-model="ctrl.CurrentTrade.Name">
                        </div>
                        <div class="flex-noshrink">
                            <button class="account-settings__tag__action button-solid button" ng-click="ctrl.SaveTrade(ctrl.CurrentTrade, 'add')" ng-disabled="ctrl.isSaving">Save</button>
                            <button class="account-settings__tag__action button-clear button" ng-click="ctrl.addingTrade = !ctrl.addingTrade" ng-disabled="ctrl.isSaving">Cancel</button>
                        </div>
                    </div>
                </div>
            </section>
    ...
    <div class="overlay" ng-if="ctrl.isDeleting" ng-cloak>
        <div class="overlay__content flex flex-col" role="dialog">
            <div class="flex flex-col flex-grow">
                <header class="overlay__header flex flex-center-center">
                    <h2 class="overlay__heading">Delete "{{ctrl.CurrentTrade.Name}}" Trade</h2>
                </header>
                ...
                <footer class="overlay__footer flex flex-between-center">
                    ...
                    <button class="button button-solid button-danger button-uppercase-small overlay__button-180" ng-click="ctrl.DeleteTrade(ctrl.CurrentTrade)">Delete</button>
                </footer>
            </div>
        </div>
    </div>
...
<script>
    ...
    <script>
        angular.module("acculynx").provider("ProductionCalendarColorList", function () {
            return {
                $get: function () {
                    return {"6e4b9b2c-95b4-4c52-946b-002f3c4439de":"#EEBCBD","b00daed5-374f-426c-bdc2-7295728b3f92":"#E58586","e19013cb-860d-416c-8318-cc863c8ee09f":"#75E78E","8c409d43-0548-4dee-be11-288e50f64b6e":"#BEECF3","a10dfd11-828e-455e-b0ac-293a60fdafd4":"#AFC6F1","c8671f73-5f84-4f3c-b575-ed259c524011":"#66E0EC","f6083a60-a6bb-4393-a315-1c08bb23b25e":"#C3EFCC","087b3727-3909-48b2-8a29-0d38324ec67b":"#EDDEC3","1be13e86-a28a-4962-863e-8f2113d79630":"#E4C793","27c31da2-16b2-47d8-88da-49dc7413be9b":"#ECC4F2"}
                }
            }
        });
    </script>
