...
<head>
    ...
    <meta charset="utf-8" />
    <title ng-init='title = "Manage Lead Sources"' ng-bind="title + ' &middot; AccuLynx'">Manage Lead Sources &middot; AccuLynx</title>
...
<header id="vue-header" class="webapp-header" data-bootstrap="{&quot;Tasks&quot;:0,&quot;TasksDueToday&quot;:0,&quot;TasksOverdue&quot;:0,&quot;Appointments&quot;:0,&quot;Reminders&quot;:0,&quot;RemindersToday&quot;:0,&quot;AtMes&quot;:16,&quot;AtMeFiltersAllowed&quot;:true,&quot;CompanyUserID&quot;:&quot;2bde0f37-77ca-4733-9b4e-aaca2078a0fd&quot;}">
    <ax-app>
        ...
        <!--help modals-->
        <ax-modal v-model="showSubmitAnIdeaModal" :component="submitAnIdeaModalView" :data="{'ViewAllIdeasURL': 'http://feedback.acculynx.com/forums/243621-general-feedback/?sso=yKNHDwZQdVXYO2TtEC5e8El1b6Oy1VlDCOhh%2f0Tr7R91OR2QJHXLAsQGiP%2fIUjymVANUIDPLiIQMWKpaZGN%2bAo0YyFRRzxcSJ7kkEtSlX47tpehNqxp22Cn%2b1lis4HuDLI6%2f1crq9vx4Lw%2fYx38waxOTOuZ%2b%2blNIpAzU%2fpD3bvmBb%2b1GRShs0gUomFjfzkeXHEimxA4VDOOCH1qEsExkRg%3d%3d'}"></ax-modal>
...
<div ng-controller="LeadSourceCtrl as ctrl">
    <div class="account-settings__parent" ng-cloak style="margin: 10px;">
        <div id="AccountSettingsMenuVue">

</div>


        <div class="flex-grow account-settings__main-content" style="max-width: 720px">
            <section class="padded widget">
                <div class="account-settings__refinements flex flex-between-center">
                    <div class="flex flex-start-center">
                        <div class="branded-search account-settings__refinement-col">
                            <svg class="icon icon-magnifying-glass icon-search"><use xlink:href="/Images/icon-sprite.svg#icon-magnifying-glass"></use></svg>
                            <input type="search" name="search" placeholder="Search" tabindex="3" ng-model="ctrl.filterText">
                        </div>
                        <label class="account-settings__refinement-col">Show:</label>
                        <div class="input-checkbox account-settings__refinement-col" tabindex="1">
                            <input class="input-checkbox__input" name="active" type="checkbox" id="filterActive" ng-model="ctrl.filterActive" ng-change="ctrl.UpdateList()">
                            <label class="input-checkbox__label--horizontal" for="filterActive">Active</label>
                        </div>
                        <div class="input-checkbox account-settings__refinement-col" tabindex="2">
                            <input class="input-checkbox__input" name="inactive" type="checkbox" id="filterInActive" ng-model="ctrl.filterInActive" ng-change="ctrl.UpdateList()">
                            <label class="input-checkbox__label--horizontal" for="filterInActive">Inactive</label>
                        </div>
                    </div>
                </div>
                <header class="widget-header">
                    <h2 class="widget-title">
                        <svg class="icon icon-default">
                            <use xlink:href="/Images/icon-sprite.svg#icon-funnel"></use>
                        </svg>
                        Lead Sources
                    </h2>
                </header>
                <div class="widget-body">
                    <p>
                        Add, edit, or remove lead sources. These sources are available when editing lead information and marketing expenses.
                    </p>

                    <header class="account-settings__tag-header flex flex-between-center">
                        <div class="flex flex-start-center">
                            <div class="flex-noshrink" style="text-align: center; width: 52px">Active</div>
                        </div>
                    </header>
                    <ul class="account-settings__tag-listing">
                        <li class="account-settings__tag flex flex-col" style="padding-left: 0; padding-right: 0;" ng-class="{'account-settings__tag--edit': leadSource.isEditing}" ng-repeat="leadSource in ctrl.FilteredList | filter:ctrl.filterParentLeadSources(ctrl.filterText) | orderBy: ['Name']">
                            <div class="flex flex-between-center" style="padding: 0 15px;">
                                <div class="flex flex-start-center">
                                    <div class="input-checkbox" style="margin-right: 12px;">
                                        <input id="checkbox-input-{{leadSource.LeadSourceID}}" ng-model="leadSource.IsActive" ng-change="ctrl.SaveLeadSource(leadSource, 'edit', true)" class="input-checkbox__input" type="checkbox" checked="checked" />
                                        <label for="checkbox-input-{{leadSource.LeadSourceID}}" class="input-checkbox__label--horizontal"></label>
                                    </div>
                                    {{leadSource.Name}}
                                </div>
                                <div class="flex-noshrink">
                                    <button class="account-settings__tag__action" ng-click="ctrl.addOrEditLeadSource(leadSource)">
                                        <svg class="icon icon-pencil">
                                            <use xlink:href="/Images/icon-sprite.svg#icon-pencil"></use>
                                        </svg>
                                    </button>
                                    <button ng-disabled="leadSource.IsUsed || leadSource.ChildrenLeadSourcesInUse()" class="account-settings__tag__action" ng-click="ctrl.openDeleteModal(leadSource)" title="{{leadSource.IsUsed ? 'Cannot Delete. Currently in use.' : 'Delete'}}">
                                        <svg class="icon icon-trash-can">
                                            <use xlink:href="/Images/icon-sprite.svg#icon-trash-can"></use>
                                        </svg>
                                    </button>
                                </div>
                            </div>
                            <ul class="m-t--xl" ng-if="leadSource.HasChildren()">
                                <li class="account-settings__tag account-settings__tag--child flex flex-between-center" ng-class="{'account-settings__tag--edit': childLeadSource.isEditing}" ng-repeat="childLeadSource in leadSource.ChildLeadSources | filter: ctrl.filterText | orderBy: ['Name']">
                                    <div class="flex flex-start-center">
                                        <div class="input-checkbox" style="margin-right: 12px;">
                                            <input id="child-checkbox-input-{{childLeadSource.LeadSourceID}}" ng-model="childLeadSource.IsActive" ng-change="ctrl.SaveLeadSource(childLeadSource, 'edit', true)" class="input-checkbox__input" type="checkbox" checked="checked" ng-disabled="!leadSource.IsActive" />
                                            <label for="child-checkbox-input-{{childLeadSource.LeadSourceID}}" class="input-checkbox__label--horizontal"></label>
                                        </div>
                                        {{childLeadSource.Name}}
                                    </div>
                                    <div class="flex-noshrink">
                                        <button class="account-settings__tag__action" ng-click="ctrl.addOrEditLeadSource(childLeadSource)">
                                            <svg class="icon icon-pencil">
                                                <use xlink:href="/Images/icon-sprite.svg#icon-pencil"></use>
                                            </svg>
                                        </button>
                                        <button ng-disabled="childLeadSource.IsUsed" class="account-settings__tag__action" ng-click="ctrl.openDeleteModal(childLeadSource)" title="{{childLeadSource.IsUsed ? 'Cannot Delete. Currently in use.' : 'Delete'}}">
                                            <svg class="icon icon-trash-can">
                                                <use xlink:href="/Images/icon-sprite.svg#icon-trash-can"></use>
                                            </svg>
                                        </button>
                    ...
                    <div class="">
                        <button class="button-v3--sm button-v3--text-blue f--16 m-t--md" tabindex="4" ng-click="ctrl.addOrEditLeadSource(null)">
                            <svg width="14" height="14" class="m-r--md ax-svg ax-svg--plus"><use xlink:href="/Images/icon-sprite.svg#icon-plus"></use></svg>
                            Add Another
                        </button>
                    </div>
    ...
    <div class="overlay" ng-if="ctrl.isDeleting" ng-cloak>
        <div class="overlay__content flex flex-col" role="dialog">
            <div class="flex flex-col flex-grow">
                <header class="overlay__header flex flex-center-center">
                    <h2 class="overlay__heading f--center">Delete "{{ctrl.CurrentLeadSource.Name}}" Lead Source</h2>
                </header>
                <div class="overlay__body" style="text-align: center;">
                    <p>Are you sure you want to delete this lead source? <span ng-if="ctrl.CurrentLeadSource.HasChildren()">Deleting this parent lead source will also delete all children sources.</span></p>
                </div>
                <footer class="overlay__footer flex flex-between-center">
                    <button class="button button-uppercase-small f--red" ng-click="ctrl.isDeleting = false" tabindex="-1">Cancel</button>
                    <button class="button button-solid button-danger button-uppercase-small overlay__button-180" ng-click="ctrl.DeleteLeadSource(ctrl.CurrentLeadSource)">Delete</button>
                </footer>
            </div>
        </div>
    </div>

    <div class="overlay" ng-if="ctrl.isManagingLeadSource" ng-cloak>
        <div class="overlay__content flex flex-col" role="dialog">
            <div class="flex flex-col flex-grow">
                <header class="overlay__header flex flex-center-center">
                    <h2 class="overlay__heading" ng-cloak>{{ ctrl.CurrentLeadSource.LeadSourceID ? 'Edit' : 'Add' }} Lead Source</h2>
                </header>
                <div class="overlay__body" style="width:600px; margin: 0 auto;">
                    <div class="form-v3-input input-row">
                        <input class="form-v3-input__input" ng-class="{'populated' : ctrl.CurrentLeadSource.Name != '' && ctrl.CurrentLeadSource.Name != null }" ng-model="ctrl.CurrentLeadSource.Name" />
                        <label class="form-v3-input__label">Name</label>
                    </div>
                    <div class="input-row" ng-if="!ctrl.CurrentLeadSource.HasChildren()">
                        <p class="f--bold m-t--lg m-b--sm">Group within a Parent Lead Source?</p>
                        <div class="form-v3-select">
                            <select class="form-v3-select__select" ng-class="{'populated' : ctrl.CurrentLeadSource.ParentLeadSourceID != null }" ng-model="ctrl.CurrentLeadSource.ParentLeadSourceID" ng-change="ctrl.setParentLeadSource(ctrl.CurrentLeadSource.ParentLeadSourceID)" name="parent-lead-sources" id="parent-lead-sources" ng-options="leadSource.LeadSourceID as leadSource.Name for leadSource in ctrl.ActiveParentLeadSources">
                                <option value=""></option>
                            </select>
                            <label for="parent-lead-sources" class="form-v3-select__label">Select Parent Lead Source</label>
                        </div>
                    </div>
                </div>
                <footer class="overlay__footer flex flex-between-center">
                    <button class="button button-uppercase-small f--red" ng-click="ctrl.cancelManagingLeadSource()" tabindex="-1">Cancel</button>
                    <button class="button button-solid button-info button-uppercase-small overlay__button-180" ng-disabled="ctrl.isSaving || ctrl.CurrentLeadSource.Name == '' || ctrl.CurrentLeadSource.Name == null" ng-click="ctrl.SaveLeadSource(ctrl.CurrentLeadSource, ctrl.CurrentLeadSource.LeadSourceID ? 'edit' : 'add')">Save</button>
                </footer>
            </div>
        </div>
    </div>
...
<script>
    ...
    <script src="https://static.alstatic.net/scriptbundle/AccountSettings-b29250d9f213b521.js" crossorigin integrity='sha512-evfoqadu2cZTUE0QHQBL6D1izokg5RLHOTGTfoYccKavxvnIw203yYPTezKdsx/ZC05XKqSRIq5F/ObC1IWTww=='></script>
