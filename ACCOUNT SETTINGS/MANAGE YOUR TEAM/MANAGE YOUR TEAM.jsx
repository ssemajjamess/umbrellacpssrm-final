...
<head>
    ...
    <meta charset="utf-8" />
    <title ng-init='title = "Manage Your Team"' ng-bind="title + ' &middot; AccuLynx'">Manage Your Team &middot; AccuLynx</title>
...
<header id="vue-header" class="webapp-header" data-bootstrap="{&quot;Tasks&quot;:0,&quot;TasksDueToday&quot;:0,&quot;TasksOverdue&quot;:0,&quot;Appointments&quot;:0,&quot;Reminders&quot;:0,&quot;RemindersToday&quot;:0,&quot;AtMes&quot;:16,&quot;AtMeFiltersAllowed&quot;:true,&quot;CompanyUserID&quot;:&quot;2bde0f37-77ca-4733-9b4e-aaca2078a0fd&quot;}">
    <ax-app>
        ...
        <!--help modals-->
        <ax-modal v-model="showSubmitAnIdeaModal" :component="submitAnIdeaModalView" :data="{'ViewAllIdeasURL': 'http://feedback.acculynx.com/forums/243621-general-feedback/?sso=yKNHDwZQdVXYO2TtEC5e8El1b6Oy1VlDCOhh%2f0Tr7R91OR2QJHXLAsQGiP%2fIUjymVANUIDPLiIQMWKpaZGN%2bAo0YyFRRzxcSJ7kkEtSlX47tpehNqxp22Cn%2b1lis4HuDLI6%2f1crq9vx4Lw%2fYx38waxOTOuZ%2b%2blNIpAzU%2fpD3bvmqC5eApab%2btgFaHaQoLzDhbw7nV9i2HIlXAoQF6BP7DA%3d%3d'}"></ax-modal>
...
<div id="page-header-wrapper" scroll-calculator="true">
    <h1 class="page-header">Manage Your Team</h1>
</div>
<div ng-controller="ManageUsersController as manageUsersCtrl" ng-init="manageUsersCtrl.initUserList()" ng-cloak class="size-containment  no-sidebar manage-users">
    <nav class="sub-nav">
        <ul>
            <li>Home</li>
            <li>Manage Team</li>
    ...
    <div class="settings-content">
        <section class="settings-utility" ng-class="{'show-filters': showFilters}">
            <!-- settings-utility section contains either actionable items or a breadcrumb -->
            
            <button id="btn-show-filters" class="btn btn-text" ng-click="showFilters = !showFilters">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100"><path d="M98.6 7.7l-35 35v52.7c0 1.8-1.1 3.5-2.8 4.2c-0.6 0.2-1.2 0.4-1.8 0.4c-1.2 0-2.3-0.4-3.2-1.3L37.7 80.5 c-0.9-0.9-1.4-2-1.4-3.2V42.8l-35-35c-1.4-1.3-1.7-3.3-1-5C1.1 1.1 2.7 0 4.5 0h90.9c1.8 0 3.5 1.1 4.2 2.8 C100.4 4.5 100 6.5 98.6 7.7z"></path></svg>
            </button>
            <a href="/manageusers/add/" class="button button-solid" tabindex="6">Invite Team Members</a>
            <label for="filterActive">Show: </label>
            <div class="inline-branded-checkbox">
                <div class="branded-checkbox" ng-keyup="manageUsersCtrl.checkboxToggle($event,'filterActive')" tabindex="1">
                    <input name="active" type="checkbox" ng-model="manageUsersCtrl.filterActive" id="filterActive" />
                    <label for="filterActive">Active</label>
                </div>
                <div class="branded-checkbox" ng-keyup="manageUsersCtrl.checkboxToggle($event,'filterInActive')" tabindex="2">
                    <input name="inactive" type="checkbox" ng-model="manageUsersCtrl.filterInActive" id="filterInActive" />
                    <label for="filterInActive">Inactive</label>
                </div>
                <div class="branded-checkbox" ng-keyup="manageUsersCtrl.checkboxToggle($event,'filterArchived')" tabindex="3">
                    <input name="archived" type="checkbox" ng-model="manageUsersCtrl.filterArchived" id="filterArchived" />
                    <label for="filterArchived">Archived</label>
                </div>
                <div class="branded-checkbox" ng-keyup="manageUsersCtrl.checkboxToggle($event,'filterDeleted')" tabindex="4">
                    <input name="deleted" type="checkbox" ng-model="manageUsersCtrl.filterDeleted" id="filterDeleted" />
                    <label for="filterDeleted">Deleted</label>
                </div>
            </div>
            <div class="branded-search">
                ...
                <input ng-model="manageUsersCtrl.filterText" type="search" name="search" placeholder="Search for a user" tabindex="5" />
            </div>
        </section>

        <section class="padded widget" ng-cloak>
            <header class="widget-header">
                <h2 class="widget-title">
                    Team Members <span class="meta">({{filteredUserList.length}} <span ng-if="filteredUserList.length == 1">Person</span><span ng-if="filteredUserList.length != 1">Members</span>)</span>
                </h2>
                <span class="widget-meta">
                    Sort by:
                    <span class="branded-select inline">
                        <select name="sort" ng-model="manageUsersCtrl.sortBy">
            ...
            <div class="widget-body">
                <table class="team-list">
                    <tbody>
                        <tr ng-repeat="user in filteredUserList = (manageUsersCtrl.userList | filter: manageUsersCtrl.filterList | orderBy:manageUsersCtrl.sortedBy())" ng-class="{'deleted': manageUsersCtrl.isDeleted(user)}">
                            <td>
                                <div class="branded-checkbox" tabindex="{{$index + 6}}" ng-keyup="manageUsersCtrl.checkboxToggleActive($event,user)">
                                    <input ng-if="manageUsersCtrl.filterLocation != ''" name="isActive{{user.UserID}}" type="checkbox" ng-checked="user.ActiveLocation.UserStatus == 1" id="isActive{{user.UserID}}" ng-click="manageUsersCtrl.toggleActive(user)" />
                                    <input ng-if="manageUsersCtrl.filterLocation == ''" name="isActive{{user.UserID}}" type="checkbox" ng-checked="user.UserStatus == 1" id="isActive{{user.UserID}}" ng-click="manageUsersCtrl.toggleActive(user)" />
                                    <label for="isActive{{user.UserID}}">Active</label>
                                </div>
                            </td>
                            <td>
                                <img ng-src="{{user.User.ProfilePicture}}NewSmall.png" alt="{{user.UserDefaultFields.DefaultDisplayName}}" class="avatar" ng-if="user.User.ProfilePicture" />
                                <div class="avatar" ng-if="!user.User.ProfilePicture">
                                    <p class="user-initials" ng-if="user.UserDefaultFields.DefaultInitials" ng-bind="user.UserDefaultFields.DefaultInitials"></p>
                                    <p class="user-initials" ng-if="!user.UserDefaultFields.DefaultInitials" al-initials="{{user.UserDefaultFields.DefaultDisplayName}}"></p>
                                </div>
                                <div class="name-block">
                                    <a ng-href="/manageusers/edit/{{user.UserID}}" ng-if="(manageUsersCtrl.filterLocation != '' && user.ActiveLocation.UserStatus != 4) || (manageUsersCtrl.filterLocation == '' && user.UserStatus != 4)" tabindex="{{$index + 7}}">
                                        {{user.UserDefaultFields.DefaultDisplayName}}
                                    </a>
                                    <div ng-if="(manageUsersCtrl.filterLocation != '' && user.ActiveLocation.UserStatus == 4) || (manageUsersCtrl.filterLocation == '' && user.UserStatus == 4)">
                                        {{user.UserDefaultFields.DefaultDisplayName}}
                                    </div>

                                    <span ng-if="manageUsersCtrl.filterLocation != ''">
                                        {{user.ActiveLocation.LocationEmail}}
                                    </span>
                                    <span ng-if="manageUsersCtrl.filterLocation == ''">
                                        {{user.User.Email}}
                                    </span>
                            ...
                            <td>
                                <!-- only show if deleted, inactive, or archived-->
                                <span ng-if="manageUsersCtrl.filterLocation != '' && user.ActiveLocation.UserStatus != 1">
                                    {{manageUsersCtrl.getUserStatus(user.ActiveLocation.UserStatus)}}
                                </span>
                                <span ng-if="manageUsersCtrl.filterLocation == '' && user.UserStatus != 1">
                                    {{manageUsersCtrl.getUserStatus(user.UserStatus)}}
                                </span>
                            </td>
                            <td>
                                <span ng-if="manageUsersCtrl.filterLocation != ''">
                                    {{manageUsersCtrl.getUserRoleText(user.ActiveLocation.Role)}}
                                </span>
                                <span ng-if="manageUsersCtrl.filterLocation == ''">
                                    {{manageUsersCtrl.getUserRoleText(user.UserDefaultFields.DefaultRole)}}
                                </span>
                            </td>
                        </tr>
                    </tbody>
    ...
    <div class="overlay" ng-if="manageUsersCtrl.showDeleteUser == true" ng-cloak>
        <div class="overlay__content overlay__content--sm flex flex-col" role="dialog">
            <button class="overlay__close-button" ng-click="manageUsersCtrl.showDeleteUser = false">
                <svg class="overlay__close-icon"><use xlink:href="/Images/icon-sprite.svg#icon-plus"></use></svg>
            </button>
            <div class="flex flex-col flex-grow">
                <header class="overlay__header flex flex-center-center">
                    <h2 class="overlay__heading flex flex-center-center">
                        <svg class="icon icon-exclamation-triangle m-r--md"><use xlink:href="/Images/icon-sprite.svg#icon-exclamation-triangle"></use></svg>
                        <span>{{manageUsersCtrl.userChangeHeaderText}}</span>
                    </h2>
                </header>
                <div class="overlay__body">
                    <p>{{manageUsersCtrl.userChangeDetailsText}}</p>
                    <p ng-if="manageUsersCtrl.userChangeLinkURL != ''"><a href="{{manageUsersCtrl.userChangeLinkURL}}">{{manageUsersCtrl.userChangeLinkText}}</a></png-if="manageUsersCtrl.userChangeLinkURL>
                </div>
                <footer class="overlay__footer flex flex-between-center">
                    <button class="button-v3--sm button-v3--text-blue f--14 f--uppercase" type="button" ng-click="manageUsersCtrl.showDeleteUser = false">Cancel</button>
                    <button class="button-v3--sm button-v3--solid-blue" ng-click="manageUsersCtrl.showDeleteUser = false">Close</button>
                </footer>
            </div>
        </div>
    </div>
...
<script>
    ...
    <script src="https://static.alstatic.net/scriptbundle/ManageUsers-44b606fdd9406ce5.js" crossorigin integrity='sha512-Zev4fXvcHDqkSqJX9v+1YIQ5iXafpnRpn0042pPFjAFtQW9umy27PvPFdDkh0/gCDCP9+lG9DoiCMd18hHW/Kg=='></script>

    <script>
        acculynx.angular.addDependency("ngResource");
        acculynx.angular.addDependency("angularLocalStorage");
    </script>
